...
BUILD=build
PREVIEW=preview
PREVIEW_PDFS = $(PREVIEW)/$(SCHEMATIC_X).pdf $(PREVIEW)/$(LAYOUT_X).pdf
PREVIEW_PNGS = $(PREVIEW)/$(SCHEMATIC_X).png $(PREVIEW)/$(LAYOUT_TOP_X).png $(PREVIEW)/$(LAYOUT_BOTTOM_X).png
PREVIEW_FILES = $(PREVIEW_PDFS) $(PREVIEW_PNGS)

default: $(PREVIEW_FILES)
...
$(PREVIEW)/$(SCHEMATIC_X).pdf: $(BUILD)/schematic.pdf | $(PREVIEW)
	cp $^ $@

$(PREVIEW)/$(LAYOUT_X).pdf: $(BUILD)/layout.pdf | $(PREVIEW)
	cp $^ $@

$(PREVIEW)/$(SCHEMATIC_X).png: $(BUILD)/schematic.png | $(PREVIEW)
	cp $^ $@

$(PREVIEW)/$(LAYOUT_TOP_X).png: $(BUILD)/layout-top.png | $(PREVIEW)
	cp $^ $@

$(PREVIEW)/$(LAYOUT_BOTTOM_X).png: $(BUILD)/layout-bottom.png | $(PREVIEW)
	cp $^ $@
